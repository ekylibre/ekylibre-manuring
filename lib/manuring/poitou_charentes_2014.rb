...
module Manuring
  class PoitouCharentes2014 < ManuringApproach
    ...
    def estimated_needs(expected_yield = nil)
      ...
      #   b = items.first.coefficient
      # end
      (expected_yield.in_kilogram_per_hectare * b / 100.0.to_d).round(2)
    end
    ...
    def soil_supplies
      ...
      if @variety_nomen && (@variety_nomen <= :poaceae || @variety_nomen <= :brassicaceae || @variety_nomen <= :medicago || @variety_nomen <= :helianthus || @variety_nomen <= :nicotiana || @variety_nomen <= :linum)
        ...
        s = if (@soil_nature_nomen <= :clay_limestone_soil || @soil_nature_nomen <= :chesnut_red_soil) || @variety_nomen > :nicotiana

              # S = Po + Mr + MrCi
              values[:soil_production] + values[:previous_cultivation_residue_mineralization] + values[:intermediate_cultivation_residue_mineralization]

            else
              # S = Pi + Ri + Mh + Mhp + Mr + MrCi + Rf
              values[:absorbed_nitrogen_at_opening] + values[:mineral_nitrogen_at_opening] + values[:humus_mineralization] +
    ...
    def estimated_input(values = {})
      ...
      # if input < 0 then 0
      input = 0.in_kilogram_per_hectare if input.to_d < 0.0
    ...
    def estimate_expected_yield
      ...
      if capacity && cultivation_varieties && (@variety_nomen <= :triticosecale || @variety_nomen <= :triticum_aestivum || @variety_nomen <= :triticum_durum || @variety_nomen <= :hordeum) && @soil_nature && (items = Manuring::Abaci::NmpPoitouCharentesAbacusTwoRow.where(cultivation_variety: cultivation_varieties.map(&:name), soil_nature: @soil_nature)) && (items = items.select { |i| i.minimum_available_water_capacity.in_liter_per_square_meter <= capacity && capacity < i.maximum_available_water_capacity.in_liter_per_square_meter }) && items.any?
      ...
      elsif cultivation_varieties && @administrative_area && (items = Manuring::Abaci::NmpFranceCultivationYield.where(cultivation_variety: cultivation_varieties.map(&:name), administrative_area: @administrative_area)) && (@variety_nomen <= :avena || @variety_nomen <= :secale || @variety_nomen <= :poaceae) && items.any?
    ...
    def estimate_previous_cultivation_residue_mineralization
      ...
      previous_crop_age = if previous_cultivation && previous_cultivation.dead_at && previous_cultivation.born_at
                            ((previous_cultivation.dead_at - previous_cultivation.born_at) / (3600 * 24 * 30)).to_i
                          else
                            1
                          end
      # find the previous crop destruction period date in format MMDD
      previous_crop_destruction_period = if previous_cultivation && previous_cultivation.dead_at
                                           previous_cultivation.dead_at.strftime('%m%d')
                                         else
                                           '0831'
                                         end
      # find the current crop implantation period date in format MMDD
      current_crop_implantation_period = if @cultivation
                                           @cultivation.born_at.strftime('%m%d')
                                         else
                                           '0315'
                                         end
      ...
      if previous_sets && previous_crop_age && previous_crop_destruction_period && current_crop_implantation_period
        ...
        variety_filtered_items = item_sorted.sort_by { |n, _item| n }
    ...
    def estimate_soil_production
      ...
      if capacity && sets
        ...
        if @variety_nomen && @variety_nomen <= :brassica_napus && plant_growth_indicator = @cultivation.density(:fresh_mass, :net_surface_area).to_d(:kilogram_per_hectare)
          ...
          plant_growth = if plant_growth_indicator <= 0.4
                           'low'
    ...
    def estimate_maximum_nitrogen_input
      ...
      if (department_item = @administrative_area) && @variety_nomen
