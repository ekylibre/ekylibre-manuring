  ...
  class PoitouCharentes2013 < ManuringApproach
    ...
    def estimated_input(values = {})
      ...
      if @variety && (@variety <= :poaceae || @variety <= :brassicaceae || @variety <= :medicago || @variety <= :helianthus || @variety <= :nicotiana || @variety <= :linum)
        ...
        if @soil_nature.include?(Nomen::SoilNature[:clay_limestone_soil]) || @soil_nature.include?(Nomen::SoilNature[:chesnut_red_soil]) && @variety && @variety > :nicotiana
          # CAU = 0.8
        ...
        input *= if @soil_nature.include?(Nomen::SoilNature[:clay_limestone_soil])
                   1.15.to_d
                 else
                   1.10.to_d
                 end
    ...
    def estimate_expected_yield
      ...
      if (items = Manuring::Abaci::NmpFranceCultivationYield.where(cultivation_variety: cultivation_varieties, administrative_area: @administrative_area || :undefined)) && items.any? && (@variety <= :avena || @variety <= :secale)
        # puts items.inspect.green
        expected_yield = items.first.expected_yield.in_quintal_per_hectare
      elsif (capacity = @available_water_capacity.in_liter_per_square_meter) && (items = Manuring::Abaci::NmpPoitouCharentesAbacusTwoRow.where(cultivation_variety: cultivation_varieties, soil_nature: @soil_nature)) && (items = items.select { |i| i.minimum_available_water_capacity.in_liter_per_square_meter <= capacity && capacity < i.maximum_available_water_capacity.in_liter_per_square_meter }) && items.any?
        # puts items.inspect.green
        expected_yield = items.first.expected_yield.in_quintal_per_hectare
      else
        variety = nil
    ...
    def estimate_absorbed_nitrogen_at_opening
      ...
      if @cultivation.blank? && @variety && (@variety <= :zea || @variety <= :sorghum || @variety <= :helianthus || @variety <= :linum || @variety <= :cannabis || @variety <= :nicotiana)
        ...
        if (count = @cultivation.leaf_count(at: @opened_at)) && activity.nature.to_sym == :cereal_crops
      ...
      if sets.any? && @soil_nature
        ...
        if items.any?
          ...
          if Activity.of_campaign(campaigns).of_families(:animal_farming).any?
            ...
            typology = if pasturing_interventions.compact.any?
                         :husbandry_with_mixed_crop
                       else
                         :husbandry
                       end
          # elsif all production on the campagin is link to a crop_set :cereals then :cereal_crop
          elsif Activity.of_campaign(campaigns).of_families(:cereal_crops).count == Activity.of_campaign(campaigns).of_families(:plant_farming).count
    ...
    def estimate_soil_production
      ...
      if (capacity = @available_water_capacity.in_liter_per_square_meter) && (sets = crop_sets.map(&:name).map(&:to_s))
    ...
    def estimate_maximum_nitrogen_input
      ...
      if (department_item = @administrative_area) && @variety
